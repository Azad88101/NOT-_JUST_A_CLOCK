...
// // let deathYear = null;
// let deathYear = 2065; //i want default you can reove
...
function getLocale() {
  ...
  // Not all browsers return the complete lang code, so we add it from the mapped values
  if (language.length === 2) {
    language =
...
let deathYear = 2065; // Default death year
...
// Previous death year logic (commented out)
/*
// Show death year dialog on page load
window.addEventListener('load', () => {
  deathYearDialog.showModal();
...
// New function to initialize death year and handle input
function initializeDeathYear() {
  // Start countdown timer with default death year
  updateCountdownTimer();
  setInterval(updateCountdownTimer, 1000);
  //////////////////////////////////////// / / /user can change the value
  // Show dialog for user to confirm or change the death year\\\\
  ...
  submitDeathYear.addEventListener("click", () => {
    const year = parseInt(deathYearInput.value);
    if (year >= 2024 && year <= 2100) {
      deathYear = year; // Update to user input
      deathYearDialog.close();
      updateCountdownTimer(); // Refresh timer with new year
    } else {
      alert("Please enter a year between 2024 and 2100");
    }
  });

  // Handle dialog close (keep default if closed without submission)
  btnDeathDialogClose.addEventListener("click", () => {
    deathYearDialog.close();
  });
}
...
rotateClockFaces();
setCurrentLangDisplay(languageFlags.find((lang) => lang.code === locale));
initializeDeathYear(); // Call the new function to handle death year
